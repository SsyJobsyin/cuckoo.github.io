...
(function build(){
  ...
  adultTopics.forEach((t,i)=>{ const lv=adultLevels[Math.floor(i/10)]; const id=`a-${lv.replace(/\s/g,'')}-${(i%10)+1}`; lessons.adult[lv].push(makeLesson(id,t,wordBank(t))); });
  const dist=[7,7,7,7,7,7,6,6,6]; let idx=0;
  kidsLevels.forEach((lv,i)=>{ lessons.kids[lv]=[]; for(let j=0;j<dist[i];j++){ const t=kidTopics[idx++]; const id=`k-${lv}-${j+1}`; lessons.kids[lv].push(makeLesson(id,t,wordBank(t))); }});
})();
...
function startGame(){
  ...
  $("#restart").onclick=startGame; $("#speakAll").onclick=()=> $$("#left .word").forEach((w,i)=>setTimeout(()=>speak(w.textContent.replace('🔤','').trim()),i*800));
  $$("#left .word").forEach(w=>w.onclick=()=>{ $$("#left .word").forEach(x=>x.classList.remove('sel')); w.classList.add('sel'); state.game.selectedLeft=w; speak(w.textContent.replace('🔤','').trim()); });
  $$("#right .word").forEach(t=>t.onclick=()=>{
    ...
    if(Lsel.dataset.key===t.dataset.key){
      state.game.score+=10; $("#score").textContent=state.game.score; Lsel.classList.remove('sel'); Lsel.classList.add('ok'); Lsel.style.pointerEvents='none'; t.classList.add('ok'); state.game.selectedLeft=null; toast('正确 +10');
    }else{
      state.game.score=Math.max(0,state.game.score-5); $("#score").textContent=state.game.score; t.classList.add('bad'); setTimeout(()=>t.classList.remove('bad'),350); toast('再试一次 -5');
    }
  });
  state.game.timer=setInterval(()=>{ state.game.time--; $("#timer").textContent=state.game.time; if(state.game.time<=0){ clearInterval(state.game.timer); toast(`时间到！得分：${state.game.score}`); }},1000);
}
